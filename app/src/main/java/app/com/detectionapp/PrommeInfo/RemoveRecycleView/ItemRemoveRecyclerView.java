package app.com.detectionapp.PrommeInfo.RemoveRecycleView;
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mDeleteBtnState == 0) {
                    ...
                    MalwareProgrammeInfoAdapter.ViewHolder viewHolder = (MalwareProgrammeInfoAdapter.ViewHolder) getChildViewHolder(view);
            ...
            case MotionEvent.ACTION_UP:
                ...
                mVelocityTracker.computeCurrentVelocity(500);//计算手指滑动的速度
                ...
                if (Math.abs(xVelocity) > 50 && Math.abs(xVelocity) > Math.abs(yVelocity)) {
                    if (xVelocity <= -50) {//左滑速度大于100，则删除按钮显示
                        deltaX = mMaxLength - upScrollX;
                        mDeleteBtnState = 2;
                    } else if (xVelocity > 50) {//右滑速度大于100，则删除按钮隐藏
                        deltaX = -upScrollX;
                        mDeleteBtnState = 1;
                    }
                } else {
                    if (upScrollX >= mMaxLength / 8) {//item的左滑动距离大于删除按钮宽度的一半，则则显示删除按钮
                        deltaX = mMaxLength - upScrollX;
                        mDeleteBtnState = 2;
                    } else if (upScrollX < mMaxLength / 8) {//否则隐藏
                        deltaX = -upScrollX;
                        mDeleteBtnState = 1;
                    }
                }
